...
public class PhoneticAnalyzersApiService
{
    ...
    public async Task<SearchResult?> SearchPersonsAsync(string name, int maxResults = 10, int? countyId = null, string? recordType = null) 
    {
        try
        {
            ...
            try 
            {
                ...
                // If validation error, bubble up with details; otherwise fall back
                if (response.StatusCode == System.Net.HttpStatusCode.BadRequest)
                {
                    var errorJson = await response.Content.ReadAsStringAsync();
                    var validation = JsonConvert.DeserializeObject<ValidationProblemDetails>(errorJson, JsonSettings);
                ...
                _logger.LogWarning("Advanced search returned status {Status}. Falling back to simple search.", (int)response.StatusCode);
                return await FallbackSimpleSearchAsync(name, maxResults, countyId, recordType);
            }
            catch (HttpRequestException hre) when (IsConnectionRefused(hre))
            {
                // Fallback to simple search on ingestion API for connection issues
    ...
    private async Task<SearchResult?> FallbackSimpleSearchAsync(string name, int maxResults, int? countyId, string? recordType)
    {
    ...
    public async Task<List<CountyInfo>?> GetCountiesAsync()
    {
        try
        {
            // Counties endpoint is provided by Search API
            var httpClient = _httpClientFactory.CreateClient("SearchApi");
